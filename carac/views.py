...
class Caracterisation(View):
    ...
    def show_scheduler(self, request):
        try:
            if len(self.cf.listConf) != 0:
                logger.debug("Send available testcases ")
            ...
            logger.error("Err scheduler {0}".format(err))
            return render(request, 'carac/home_v2.html', {'listConf': self.cf.listConf, 'channels': self.channels,
                                                          'temperature': [], "listTc": [],
                                                          'clim_chamber_delay': 0})
    ...
    def load(self, request):
        if request.is_ajax():
            ...
            listTc = self.cf.get_testcases()
            logger.debug('Tc2play done from : {0}'.format(self.cf.nameConf))
    ...
    def get_parametersCmd(self, request):
        if request.is_ajax():
            ...
            logger.debug('commands: {0} '.format(form))
            form["nameCarac"] = str(form["type"] + form["nameCarac"])
            self.cmd = form
            return True
        else:
            logger.debug("Err POST commands".format(request))
    ...
    def write_schedule(self, request):
        if request.is_ajax():
            ...
            logger.debug("GET from AJAX Tc2play")
        else:
    ...
    def counter(self, request):
        return JsonResponse(self.taskScheduler.progress)
    ...
    def start(self, request):
        if self.get_parametersCmd(request) == True:
            ...
            logger.info("Scheduler is not valide")
