...
class Configuration(View):
    ...
    def __init__(self):
        ...
        try:
            self.get_listConf()
            self.get_conf()
            logger.info('init with the conf: {0}'.format(self.nameConf))
        except KeyError:
    ...
    def get_nameConf(self, request):
        if request.is_ajax:
            try:
                ...
                logger.debug("GET POST: {0}".format(self.nameConf))
            except KeyError as err:
                logger.debug("no names selected {0}".format(err))
        elif request.method == 'POST':
            try:
                if 'configname' in request.POST.keys():
                    self.nameConf = str(dict(request.POST)['configname'][0])
                    logger.debug("POST: {0}".format(self.nameConf))
                else:
                    logger.error("POST: get_nameConf 'configname' is not a key")
                    HttpResponse("Configuration file is not define")
            except KeyError as err:
                logger.debug("POST: no names selected {0}".format(err))
        else:
            try:
                if self.listConf != []:
                    self.nameConf = self.listConf[0]
            except ValueError as err:
                logger.error("listConf is empty {0}".format(err))
    ...
    def get_conf(self):
        ...
        if self.nameConf in self.listConf:
            ...
            logger.info("configuration:{0}".format(self.nameConf))
        elif len(self.listConf) != 0:
            self.nameConf = self.listConf[0]
            logger.debug("init with the first conf of the list")
        else:
            logger.error("collection mongo is empty")
    ...
    def read(self, request):
        logger.debug("read : {0} ".format(self.configs.keys()))
        if '_id' in self.configs.keys():
            del self.configs['_id']
    ...
    def get_confType(self):
        ...
        try:
            ...
            logger.error(' testcase .json does not exist: {0}'.format(err))
    ...
    def save(self, request):
        if request.is_ajax():
            ...
            logger.debug("save: {0}".format(configToWrite))
            logger.info('configuration {0} saved'.format(name))
            file = configToWrite['data'][0]
            json_file = json.loads(file)
            if name in self.listConf:
                self.delete(request)
                logger.info('configuration {0} modified'.format(name))
                print('The configuration', name, 'has been modified')
            self.db.writeDataBase(json_file, configToWrite['conf'][0])
            self.init(request)
        else:
            logger.error("POST: save conf failed")
    ...
    def delete(self, request):
        try:
            if request.is_ajax():
                ...
                logger.debug("configuration {0} delete".format(confName_to_delete))
                if confName_to_delete in self.listConf:
                    self.db.delete_collection(confName_to_delete)
                    logger.info("file {0} is delete".format(confName_to_delete))
                    self.init(request)
                    return render(request, 'config/home.html', {'listConf': self.listConf})
        except IOError as err:
            logger.error("delete error : {0}".format(err))
    ...
    def add(self, request):
        ...
        if request.method == 'POST':
            ...
            if fileform.is_valid():
                ...
                self.init(request)
                logger.debug("fichier {0} enregistré sur la base de donnée".format(file_name))
                return render(request, 'config/home.html', {'listConf': self.listConf})
        else:
